...
use crate::error::SolanaPythonError;
use crate::instruction::accounts::CreateAccounts;
use crate::instruction::{CreateArgs, SolanaPythonInstruction};
...
pub fn process_instruction<'a>(
    ...
    let instruction: SolanaPythonInstruction =
        SolanaPythonInstruction::try_from_slice(instruction_data)?;
    match instruction {
        SolanaPythonInstruction::Create(args) => {
...
fn create<'a>(accounts: &'a [AccountInfo<'a>], args: CreateArgs) -> ProgramResult {
    ...
    if *ctx.accounts.system_program.key != system_program::id() {
        return Err(SolanaPythonError::InvalidSystemProgram.into());
    }
